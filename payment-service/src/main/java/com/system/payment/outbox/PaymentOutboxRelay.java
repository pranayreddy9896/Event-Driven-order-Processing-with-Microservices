package com.system.payment.outbox;
...
public class PaymentOutboxRelay {
    ...
    public void publishPendingOutboxEvents() {
        ...
        log.debug("Payment Outbox Relay found {} pending events to publish", pendingEvents.size());
        ...
        for (OutboxEvent event : pendingEvents) {
            try {
                ...
                kafkaTemplate.send(record).get(); // Synchronous get to ensure Kafka ACK before marking SENT

                event.setStatus(OutboxStatus.SENT);
                event.setProcessedAt(Instant.now());
                outboxEventRepository.save(event);

                log.info("Payment Outbox Relay published event [id={}, topic={}, aggId={}]",
