package com.system.order.outbox;
    ...
    public void publishPendingOutboxEvents() {
        ...
        for (OutboxEvent event : pendingEvents) {
            try {
                ...
                log.info("Order Outbox Relay successfully published event [id={}, topic={}, aggId={}]",
                        event.getId(), event.getTopic(), event.getAggregateId());
            } catch (Exception e) {
                log.error("Failed to publish order outbox event {}: {}", event.getId(), e.getMessage());
